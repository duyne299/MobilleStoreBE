...
const FlashSale = () => {
    ...
    // Load sản phẩm từ category 4-11 và lấy thông tin warehouse
    useEffect(() => {
    ...
    const nextSlide = () => {
        setCurrentSlide((prev) => (prev + 1 >= Math.ceil(products.length / 2) ? 0 : prev + 1));
    };

    const prevSlide = () => {
        setCurrentSlide((prev) => (prev - 1 < 0 ? Math.ceil(products.length / 2) - 1 : prev - 1));
    };
    ...
    if (loading && products.length === 0) {
        return (
            <div className="h-auto from-gray-50 to-white py-8 sm:px-6 lg:px-8">
                                ...
                                >
                                    {Array.from({ length: Math.ceil(products.length / 2) }).map((_, slideIndex) => (
                                        <div key={slideIndex} className="w-full flex-shrink-0 flex gap-4 px-2">
                                            {products.slice(slideIndex * 2, slideIndex * 2 + 2).map((product, idx) => (
                                                <div key={product.proId} className="w-1/2 flex-shrink-0">
                                                    <ProductCard
                                                        product={product}
                                                        index={idx}
                                                        formatPrice={formatPrice}
...
const ProductCard = ({ product, index, formatPrice, calculateDiscount, router }: {
    product: any;
    index: number;
    formatPrice: (price: number) => string;
    calculateDiscount: (price: number, index: number) => { newPrice: number; discount: string };
    router: any;
}) => {
        ...
        >
            ...
            <div className="p-1">
                <div
                    ...
                    style={{
                        backgroundImage: 'url(https://fptshop.com.vn/img/Price-GVGS.png?w=1920&q=75)',
                ...
                <button
                    ...
                    className={`w-full py-2.5 rounded-full text-sm font-semibold transition-all ${hasStock

...
import {
    Smartphone, Headphones, Apple, Newspaper, BadgePercent, Star, BookOpenText, Tablet
} from 'lucide-react';
...
const brandCategories = [
    ...
    { id: 'lg', name: 'LG', icon: <div className="w-5 h-5 bg-red-600 rounded-full flex items-center justify-center text-white font-bold text-xs">LG</div> },
    { id: 'samsung', name: 'Samsung', icon: <div className="w-5 h-5 bg-blue-600 rounded-sm flex items-center justify-center text-white font-bold text-xs">S</div> },
    { id: 'xiaomi', name: 'Xiaomi', icon: <div className="w-5 h-5 bg-orange-500 rounded-sm flex items-center justify-center text-white font-bold text-xs">Mi</div> },
    { id: 'garmin', name: 'Garmin', icon: <div className="w-5 h-5 bg-black rounded-sm flex items-center justify-center text-white font-bold text-xs">G</div> },
];
...
export default function CategoryMenu({ isPinned: externalPinned, onPinChange }: CategoryMenuProps) {
    const [activeCategory, setActiveCategory] = useState<number | string>(mainCategories[0].id);
    const menuRef = useRef<HTMLDivElement>(null);
    const hoverTimeoutRef = useRef<NodeJS.Timeout | null>(null);
    const { categories, loading: loadingCategories, fetchCategories } = useCategories(100);
    const { products, loading: loadingProducts, getByCategoryId, fetchProducts } = useProducts(1000);
    const [subCategoryProducts, setSubCategoryProducts] = useState<{ [key: number]: any[] }>({});
    ...
    // Khi activeCategory thay đổi, fetch danh sách sản phẩm cho subcategories
    useEffect(() => {
        const loadProductsBySub = async () => {
            if (!categories.length) return;

            setLoadingSubProducts(true);

            const subCats = categories.filter(cat => cat.parentId === activeCategory && cat.isActive);
            const result: { [key: number]: any[] } = {};

            await Promise.all(
                subCats.map(async (sub) => {
                    try {
                        const res = await getByCategoryId(sub.categoryId);
                        
                        // FIX: Kiểm tra cấu trúc response và lấy đúng data
                        let productList = [];
                        ...
                        result[sub.categoryId] = productList.slice(0, 10);
                    } catch (err) {
                        result[sub.categoryId] = [];
                    }
                })
            );
        ...
        loadProductsBySub();
    }, [activeCategory, categories, getByCategoryId]);
    ...
    const rightContent = useMemo(() => {
        ...
        if (accessoriesCategory) {
            ...
            content[3] = [
                ...
                subs.slice(Math.ceil(subs.length / 3), Math.ceil(subs.length * 2 / 3)).map(sub => ({
                    name: sub.categoryName,
                    products: []
                })),
                            ...
                            <div className="p-3 md:p-4 space-y-0.5">
                                {mainCategories.map((category) => (
                                    <div
                                        ...
                                        className={`flex items-center px-3 py-2.5 cursor-pointer transition-colors ${activeCategory === category.id ? 'bg-gray-100' : 'hover:bg-gray-100'
                                            }`}
                                    >
                                        <span className="text-gray-700 mr-3">{category.icon}</span>
                                        <span className="text-sm font-normal text-gray-800">{category.name}</span>
                                    </div>
                                ))}
                            </div>
                                ...
                                <div className="grid grid-cols-3 gap-3 w-full">
                                    {brandCategories.map((brand) => (
                                        <div
                                            ...
                                            className={`flex items-center gap-2 w-full py-2.5 hover:bg-gray-100 cursor-pointer transition-colors rounded-md ${activeCategory === brand.id ? 'bg-gray-100' : ''
                                                }`}
                                        >
                        ...
                        <div className="w-full md:w-2/3 p-4 md:p-5 overflow-y-auto">
                            {loadingSubProducts ? (
                                ...
                                <>
                                    ...
                                    {rightContent[activeCategory] && rightContent[activeCategory].length > 0 ? (
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            {rightContent[activeCategory].map((group: any[], idx: number) => (
                                                                ...
                                                                {item.products && item.products.length > 0 ? (
                                                                    item.products.map((product: any, j: number) => (
                                                                        <span
                                                                            ...
                                                                            title={product.proName || product.name || 'Sản phẩm'}
                                                                        >
                                                                            {product.proName || product.name || 'Sản phẩm không có tên'}
                                                                        </span>
                                                                    ))
                                                                ) : (

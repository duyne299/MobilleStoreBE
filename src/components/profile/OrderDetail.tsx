...
export default function OrderDetail({ orderId, onBack }: OrderDetailProps) {
  const { getOrder, getOrderDetailsByOrder, loading, error } =
    useOrders(999999);
  ...
  useEffect(() => {
    ...
    const fetchOrderDetail = async () => {
      try {
        ...
        const productPromises = (Array.isArray(details) ? details : []).map(
          async (detail: any) => {
            if (detail.option?.optionId) {
              try {
                const product = await getProductByOptionId(
                  detail.option.optionId,
                );
                return { optionId: detail.option.optionId, product };
              } catch (error) {
                console.error(
                  `Error fetching product for option ${detail.option.optionId}:`,
                  error,
                );
                return null;
              }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6 border-b pb-4">
        ...
        <span
          className={`flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium ${currentStatus.bgColor} ${currentStatus.color}`}
        >
          <StatusIcon size={14} />
          {currentStatus.label}
        </span>
                ...
                {orderDetails.map((detail, index) => {
                  ...
                  const imageUrl = (() => {
                    if (product?.images && product.images.length > 0) {
                      ...
                      return (
                        product.images.find((img: any) => img.isCover === true)
                          ?.imageUrl ||
                  ...
                  return (
                    <div
                      ...
                      className="flex gap-4 px-4 py-2 bg-gray-50 rounded-lg"
                    >
                      ...
                      <div className="text-right">
                        ...
                        {option?.originalPrice &&
                          option.originalPrice > detail.price && (

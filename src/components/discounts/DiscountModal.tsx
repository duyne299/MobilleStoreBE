...
export default function DiscountModal({
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.value || Number(formData.value) <= 0) {
            setToast({ message: 'Vui lòng nhập giá trị giảm giá hợp lệ', type: 'error' });
            return;
        }
        ...
        if (formData.discountType === 'PERCENT' && Number(formData.value) > 100) {
            setToast({ message: 'Phần trăm giảm giá không được vượt quá 100%', type: 'error' });
            return;
        }
        ...
        if (formData.minOrderValue && Number(formData.minOrderValue) < 0) {
            setToast({ message: 'Giá trị đơn tối thiểu không được âm', type: 'error' });
            return;
        }
        ...
        try {
            // Giả lập API call
            console.log('Saving discount:', payload);
            setToast({ 
                message: discount ? 'Cập nhật mã giảm giá thành công!' : 'Thêm mã giảm giá thành công!', 
                type: 'success' 
            });
    ...
    const handleKeyDown = (e: any) => {
        ...
        if (e.target.type === 'number' && (e.key === '-' || e.key === '+' || e.key === 'e' || e.key === 'E')) {
            e.preventDefault();
        }
    };
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                onChange={handleChange}
                                                placeholder={formData.discountType === 'PERCENT' ? 'Ví dụ: 20' : 'Ví dụ: 50000'}
                                                min="0"
                                                step="0.01"

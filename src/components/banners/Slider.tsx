        ...
        {topBanner && (
          <div className="mb-4 overflow-hidden bg-gray-100 rounded-lg">
            <img
              src={
                topBanner.imageUrl
                  ? topBanner.imageUrl.startsWith("http")
                    ? topBanner.imageUrl
                    : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${topBanner.imageUrl}`
                  : "/default-image.png"
              }
            ...
            {isMobile ? (
              ...
              <AnimatePresence mode="wait" custom={direction}>
                <motion.div
                  ...
                  onClick={() => {
                    if (bottomBanners[currentIndex]?.linkTarget) {
                      window.open(
                        bottomBanners[currentIndex].linkTarget,
                        "_blank",
                      );
                    }
                  }}
                >
                  <img
                    src={
                      bottomBanners[currentIndex]?.imageUrl
                        ? bottomBanners[currentIndex].imageUrl!.startsWith(
                            "http",
                          )
                          ? bottomBanners[currentIndex].imageUrl!
                          : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${bottomBanners[currentIndex].imageUrl}`
                        : "/default-image.png"
                    }
                    alt={
                      bottomBanners[currentIndex]?.title ||
                  ...
                  <div className="relative overflow-hidden rounded-2xl">
                    <AnimatePresence mode="popLayout" custom={direction}>
                      <motion.div
                        ...
                        onClick={() => {
                          const target =
                            bottomBanners[currentIndex + 1]?.linkTarget ??
                            undefined;
                          if (target) {
                      ...
                      >
                        <img
                          src={
                            bottomBanners[currentIndex + 1]?.imageUrl
                              ? bottomBanners[
                                  currentIndex + 1
                                ].imageUrl!.startsWith("http")
                                ? bottomBanners[currentIndex + 1].imageUrl!
                                : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${bottomBanners[currentIndex + 1].imageUrl}`
                              : "/default-image.png"
                          }
                          alt={
                            bottomBanners[currentIndex + 1]?.title ||

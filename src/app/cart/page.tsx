...
export default function CartPage() {
    ...
    const checkedItemsCount = enrichedItems.filter(item => item.isChecked).length;
    const selectAll = enrichedItems.length > 0 && checkedItemsCount === enrichedItems.length;

    const showToast = (type: "success" | "error", message: string) => {
        setToast({ type, message });
    };

    const handleUpdateQuantity = async (itemId: number, delta: number, currentQuantity: number, availableQuantity: number) => {
        ...
        try {
            ...
            setTempQuantities(prev => ({ ...prev, [itemId]: newQuantity.toString() }));
    ...
    const handleQuantitySubmit = async (itemId: number, availableQuantity: number) => {
        ...
        if (value === '' || isNaN(numValue)) {
            ...
            if (item) {
                setTempQuantities(prev => ({ ...prev, [itemId]: item.quantity.toString() }));
            }
            return;
        }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, itemId: number, availableQuantity: number) => {
        if (e.key === 'Enter') {
            e.preventDefault();
    ...
    const handleClearSelected = async () => {
        ...
        if (window.confirm(`Bạn có chắc muốn xóa ${checkedItems.length} sản phẩm đã chọn?`)) {
            try {
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            ...
                            >
                                <label className="flex items-center gap-3 cursor-pointer">
                                    ...
                                    />
                                    <span className="text-gray-700">Chọn tất cả ({enrichedItems.length})</span>
                                </label>
                                <button
                                                ...
                                                <div className="flex items-center justify-between flex-wrap gap-4">
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        <div className="flex items-center gap-2">
                                                            <button
                                                                onClick={() => handleUpdateQuantity(item.itemId, -1, item.quantity, item.availableQuantity)}
                                                            ...
                                                            <input
                                                                ...
                                                                onBlur={() => handleQuantitySubmit(item.itemId, item.availableQuantity)}
                                                                onKeyDown={(e) => handleKeyDown(e, item.itemId, item.availableQuantity)}
                                                                disabled={loading}
                            ...
                            >
                                <div className="bg-white rounded-lg p-4">
                                    ...
                                    <div className="bg-red-50 rounded-lg p-3 flex items-start gap-2">
                                        ...
                                        <span className="text-sm text-gray-700">Chọn hoặc nhập ưu đãi</span>
                                    </div>
                                    ...
                                    >
                                        {loading ? 'Đang xử lý...' : `Xác nhận đơn (${checkedItemsCount})`}
                                    </motion.button>
                                </div>
                            </motion.div>
                        </div>

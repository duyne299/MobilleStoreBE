...
export default function CheckoutPage() {
  ...
  const { updateStock } = useProductVariants();
  ...
  const calculateDiscountAmount = () => {
    ...
    if (appliedDiscount.discountType === "PERCENT") {
      const discountAmount =
        (checkoutData.totalAmount * appliedDiscount.value) / 100;
  ...
  const createOrderAndDetails = async () => {
    ...
    const orderData = {
      ...
      orderType: (deliveryMethod === "store" ? "PICKUP" : "DELIVERY") as
        | "PICKUP"
        | "DELIVERY",
    ...
    // Giảm tồn kho
    const storeIdForWarehouse =
      deliveryMethod === "store"
        ? selectedStore!
        : stores.find((s) => s.isActive)?.storeId || 1;

    const reduceStockPromises = checkoutData.products.map((product) =>
      updateStock(product.optionId, -product.quantity),
    );
    ...
    if (
      appliedDiscount &&
      appliedDiscount.usageLimit != null &&
      appliedDiscount.usageLimit > 0
    ) {
      try {
                      ...
                      src={
                        product.coverImage
                          ? product.coverImage.startsWith("http")
                            ? product.coverImage
                            : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${product.coverImage}`
                ...
                <div className="flex justify-between">
                  <div>
                    <p className="font-medium">
                      {checkoutData.customerInfo.fullName}
                    </p>

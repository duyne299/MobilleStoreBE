...
export default function EditProductPage() {
    ...
    const { importItem, getByOption } = useWarehouses();
    ...
    useEffect(() => {
        ...
        const loadProduct = async () => {
            try {
                const product = await getProductBySlug(Array.isArray(slug) ? slug[0] : slug);
                if (!product) {
                ...
                // Set basic info
                setProductName(product.proName || '');
                setDescription(product.description || '');

                // Xử lý brand và category - backend trả về object
                setSelectedBrand(product.brand?.brandId?.toString() || '');
                setSelectedCategory(product.category?.categoryId?.toString() || '');

                // Xử lý images - backend trả về array với {imageId, imageUrl, isCover}
                if (product.images && product.images.length > 0) {
                    const imageUrls = product.images.map((img: any) => img.imageUrl);
                    const coverIdx = product.images.findIndex((img: any) => img.isCover);
                ...
                if (product.specification) {
                    const specData = product.specification;
                    setSpec({
                ...
                // Load options (variants) - backend trả về trong options array
                if (product.options && product.options.length > 0) {
                    const loadedVariants = await Promise.all(
                        product.options.map(async (option: any) => {
                            // Load warehouse/inventory data cho mỗi option
                            let warehouseData = {
                                storeId: null,
                                quantity: 0,
                                importPrice: 0,
                            ...
                            try {
                                const warehouseItems = await getByOption(option.optionId);
                                if (warehouseItems) {
                                    const warehouseItem = Array.isArray(warehouseItems)
                                        ? warehouseItems[0]
                                        : warehouseItems;

                                    console.log(`📊 Processing warehouse item:`, warehouseItem);

                                    if (warehouseItem) {
                                        warehouseData = {
                                            storeId: warehouseItem.store?.storeId || warehouseItem.storeId || null,
                                            quantity: warehouseItem.quantity || 0,
                                            importPrice: warehouseItem.importPrice || 0,
                                            salePrice: warehouseItem.baseSalePrice || warehouseItem.salePrice || 0
                                        };
                                    }
                                }
                            } catch (err) {
                                console.error(`Error loading inventory for option ${option.optionId}:`, err);
                            }
                            ...
                            return {
                                ...
                                isActive: true, // Backend không trả về isActive, mặc định true
                                ...warehouseData
                            };
                        })
                    );
    ...
    const handleSave = async () => {
        ...
        if (!productName.trim()) return showToast("error", "Vui lòng nhập tên sản phẩm");
        ...
        for (let i = 0; i < variants.length; i++) {
            ...
            try {
                ...
                // Update product
                await updateProduct(Array.isArray(slug) ? slug[0] : slug, productPayload, selectedFiles);
                ...
                // Update variants + warehouse
                for (const v of variants) {
                    if (v.optionId) {
                        await updateVariant(v.optionId, {
                            color: v.color,
                            rom: v.rom,
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    >
                        <div className="flex flex-wrap gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
                            <Link href={"/admin"} className="hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors">
                                Dashboard
                            </Link>
                            <span>/</span>
                            <Link href={"/admin/products"} className="hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer transition-colors">
                                Sản phẩm
                            </Link>
                            <span>/</span>
                            <span className="text-gray-800 dark:text-gray-200 font-medium">Chỉnh sửa sản phẩm</span>
                        </div>
                    ...
                    >
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <label className="flex flex-col">
                                            ...
                                            >
                                                ...
                                                {!loadingBrand && !errorBrand && brands?.length > 0 &&
                                                    brands.map((brand) => (
                                        ...
                                        <label className="flex flex-col">
                                            ...
                                            >
                                                ...
                                                {!loadingCat && !errorCat && categories?.length > 0 &&
                                                    categories.map((cat) => (
                        ...
                        >
                            <div className="p-5 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800 flex items-center justify-between">
                                <p className="text-gray-900 dark:text-white text-lg font-semibold">Thông số kỹ thuật</p>
                                <motion.button
                                                ...
                                                <div className="space-y-4">
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        ...
                                                        <label className="flex flex-col">
                                                            ...
                                                            <input
                                                                value={variant.rom}
                                                                onChange={(e) => updateVariantField(index, 'rom', e.target.value)}
                                                                className="w-full rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 h-11 px-4 text-sm transition-all"
                                                        ...
                                                        <label className="flex flex-col">
                                                            ...
                                                            <input
                                                                ...
                                                                onChange={(e) => updateVariantField(index, 'extraPrice', Number(e.target.value))}
                                                                className="w-full rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 h-11 px-4 text-sm transition-all"
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                                            <label className="flex flex-col">
                                                                ...
                                                                <select
                                                                    ...
                                                                    onChange={(e) => updateVariantField(index, "storeId", Number(e.target.value))}
                                                            ...
                                                            <label className="flex flex-col">
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={(e) => updateVariantField(index, 'importPrice', Number(e.target.value))}
                                                                    className="w-full rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 h-11 px-4 text-sm transition-all"
                                                            ...
                                                            <label className="flex flex-col">
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={(e) => updateVariantField(index, 'salePrice', Number(e.target.value))}
                                                                    className="w-full rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 h-11 px-4 text-sm transition-all"
                                                            ...
                                                            <label className="flex flex-col">
                                                                ...
                                                                <input
                                                                    ...
                                                                    onChange={(e) => updateVariantField(index, 'quantity', Number(e.target.value))}
                                                                    className="w-full rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 h-11 px-4 text-sm transition-all"
                        ...
                        >
                            ...
                            <motion.button
                                onClick={handleSave}
                                disabled={saving}
                                className={`px-8 py-2.5 rounded-xl text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition-all bg-blue-600 ${saving ? 'opacity-70 cursor-not-allowed' : 'hover:bg-blue-700'
                                    }`}

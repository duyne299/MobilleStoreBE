...
export default function ProductManagement() {
    ...
    const [confirmDelete, setConfirmDelete] = useState<{ show: boolean; proId?: number; proName?: string }>({ show: false });
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            showToast("error", err.message || `Xóa sản phẩm "${confirmDelete.proName}" thất bại!`);
        } finally {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    <div className="flex flex-wrap gap-2 text-sm text-gray-500 dark:text-gray-400 mb-3">
                        ...
                        <span>/</span>
                        <span className="text-gray-800 dark:text-gray-200 font-medium">Sản phẩm mới</span>
                    </div>
                            ...
                            <table className="w-full">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap">Hình ảnh</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600">Tên sản phẩm</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600">Thương hiệu</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap">Danh mục</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap ">Tồn kho</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap">Đã bán</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap">Đánh giá TB</th>
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {products.map((product, idx) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4">
                                                <div className="w-10 h-10 bg-gray-100 rounded-lg overflow-hidden">
                                                    {product.images?.find((img: any) => img.isCover)?.imageUrl ? (
                                                        <img
                                                            src={`${process.env.NEXT_PUBLIC_API_URL}${product.images.find((img: any) => img.isCover).imageUrl}`} // ghép base URL
                                                            alt={product.proName}
                                                            className="w-full h-full object-cover"
                                                        />
                                                    ) : (
                                                        <span className="w-full h-full flex items-center justify-center text-gray-400">📦</span>
                                                    )}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-4 py-4">
                                                <span className="text-sm text-gray-600">{product.brand?.brandName || 'N/A'}</span>
                                            </td>
                                            <td className="px-4 py-4">
                                                <span className="text-sm text-gray-600">{product.category?.categoryName || 'N/A'}</span>
                                            </td>
                                            <td className="px-4 py-4 text-center">
                                                <span className="text-sm text-gray-900">{product.totalQuantity}</span>
                                            </td>

                                            <td className="px-4 py-4 text-center">
                                                <span className="text-sm text-gray-900">{product.soldQuantity || 0}</span>
                                            </td>
                                            <td className="px-4 py-4 text-center">
                                            ...
                                            <td
                                                ...
                                                onDoubleClick={async () => {
                                                    try {
                                                        ...
                                                        await changeProductStatus(product.proId, !product.isActive);
                                                    } catch (err) {
                                                        console.error(err);
                                                    }
                                                }}
                                            >
                                                <span
                                                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${product.isActive
                                            ...
                                            <td className="px-4 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <motion.button
                                                        onClick={() => handleDeleteClick(product.proId, product.proName)}

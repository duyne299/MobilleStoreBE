...
export default function CategoryManagement() {
    ...
    const handleDeleteClick = (categoryId: number, categoryName: string) => {
        setConfirmDelete({ show: true, categoryId, categoryName });
    };
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            showToast(
                "success",
                `Xóa danh mục "${confirmDelete.categoryName}" thành công!`
            );
            searchCategories(searchTerm);
        } catch (err: any) {
            showToast(
                "error",
                err.message || `Xóa danh mục "${confirmDelete.categoryName}" thất bại!`
            );
        } finally {
        ...
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                >
                    <div className="overflow-x-auto">
                        {loading ? (
                            <div className="p-4 text-center text-gray-500">Đang tải danh mục...</div>
                        ) : error ? (
                            ...
                            <table className="w-full">
                                <thead className="bg-gray-50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600">Tên danh mục</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600 whitespace-nowrap">Danh mục cha</th>
                                        <th className="px-4 py-4 text-left text-sm font-semibold text-gray-600">Mô tả</th>
                                    ...
                                    {categories.map((cat, idx) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-4 text-sm text-gray-600">
                                                {cat.parentId
                                                    ? cat.parentId === 2
                                                        ...
                                                        : cat.parentId === 3
                                                            ? "Bài viết"
                                                            : categories.find(c => c.categoryId === cat.parentId)?.categoryName || `ID: ${cat.parentId}`
                                                    : "Không có"}
                                            </td>
                                            ...
                                            <td
                                                ...
                                                onDoubleClick={() =>
                                                    updateCategoryStatus(cat.slug, !cat.isActive)
                                                }
                                            >
                                                <span
                                                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${cat.isActive
                                            ...
                                            <td className="px-4 py-4">
                                                <div className="flex items-center gap-2">
                                                    <motion.button
                                                        ...
                                                        onClick={() => {
                                                            setEditingCategory(cat);  // gán danh mục hiện tại
                                                            setIsModalOpen(true);
                                                        }}
                                                    >
                                                    ...
                                                    <motion.button
                                                        onClick={() =>
                                                            handleDeleteClick(cat.categoryId, cat.categoryName)
                                                        }
            ...
            <CategoryModal
                ...
                onClose={() => {
                    ...
                    setEditingCategory(null); // reset khi đóng modal
                }}
                setToast={(toast) => showToast(toast.type, toast.message)}
                category={editingCategory}  // thêm prop category
                onSuccess={(updatedCategory) => {
                    searchCategories(searchTerm); // reload danh sách
                }}
            />

            <ConfirmToast
                show={confirmDelete.show}
                message={`Bạn có chắc muốn xóa danh mục "${confirmDelete.categoryName}" không?`}

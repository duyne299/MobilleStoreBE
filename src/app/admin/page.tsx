...
export default function Dashboard() {
  ...
  const averageRating = useMemo(() => {
    const totalRating = products.reduce(
      (sum, p) => sum + (p.rating || 0) * (p.totalReviews || 0),
      0,
    );
      ...
      .map((product, idx) => ({
        ...
        percentage: Math.round(
          ((product.soldQuantity || 0) /
            Math.max(...products.map((p) => p.soldQuantity || 0))) *
            100,
        ),
      }));
  }, [products]);
              ...
              {topProducts.map((product, idx) => (
                ...
                >
                  <div className="flex items-center space-x-4 flex-1">
                    ...
                    <div className="w-14 h-14 bg-gradient-to-br rounded-xl overflow-hidden flex items-center justify-center shadow-lg border border-gray-200">
                      <img
                        src={
                          product.image
                            ? product.image.startsWith("http")
                              ? product.image
                              : `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080"}${product.image.startsWith("/") ? "" : "/"}${product.image}`
